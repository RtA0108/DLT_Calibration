...
public class EntropySaliency : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Step 2: Select top 6 salient points (highest saliency scores)
        saliencyPoints = SelectTopSalientPoints(saliency, 6); // Select top 6 points

        // Step 3: Visualize saliency points
    ...
    /// <summary>
    /// Select top N salient points strictly based on highest saliency scores.
    /// </summary>
    List<Vector3> SelectTopSalientPoints(float[] saliency, int numToSelect)
    {
        // Step 1: Select top N vertices with the highest saliency
        ...
        return topIndices.Select(idx => vertices[idx]).ToList();
    }
    ...
    void VisualizeSaliencyPoints(List<Vector3> salientPoints)
    {
        foreach (var point in salientPoints)
        {
            ...
            marker.GetComponent<Renderer>().material.color = Color.green; // Changed to green for clarity

            Destroy(marker.GetComponent<Collider>()); // Optional
        }

        Debug.Log("Top Salient Points Visualized:");
        foreach (var point in salientPoints)
        {
            Debug.Log(point);
        }
